...
static uint64_t physical_addresses[MAX_PAGES_TO_RECORD];
...
ssize_t example_read (struct file *filp, char __user * userdata, size_t size, loff_t *offset)
{
	...
	for(i=0;i<nr_pages;i++) {
		...
		mem->table[i].physaddr = physical_addresses[i];
...
ssize_t example_write (struct file *filp, const char __user * userdata, size_t size, loff_t *offset)
{
	...
	while(length > 0) {
		...
		physical_addresses[i] = ((uint64_t)page_to_phys(temppage[0])) + address_offsets[i];
